...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSLog(@"%@", [[NSUserDefaults standardUserDefaults] objectForKey:@"ProfImage"]);
    
    lblFollowerCount.text = [NSString stringWithFormat:@"%@", [[NSUserDefaults standardUserDefaults] objectForKey:@"follower_count"]];
    lblFollowingCount.text = [NSString stringWithFormat:@"%@", [[NSUserDefaults standardUserDefaults] objectForKey:@"following_count"]];
    lblReviewCount.text = [NSString stringWithFormat:@"%@", [[NSUserDefaults standardUserDefaults] objectForKey:@"review_count"]];
}
...
- (void) imagePickerController:(UIImagePickerController *)picker1 didFinishPickingImage:(UIImage *)imageSel editingInfo:(NSDictionary *)editingInfo {
    ...
    if (clickIndex == 1) {
        imgProfileCover.image = [editingInfo objectForKey:UIImagePickerControllerOriginalImage];
    } else if (clickIndex == 2){
        imgProfile.image = [editingInfo objectForKey:UIImagePickerControllerOriginalImage];
    }
}
...
- (IBAction)btnRateProfileAction:(id)sender
{
    ...
    viewController.isOtherUser = NO;
    viewController.profileID = [[NSUserDefaults standardUserDefaults] objectForKey:@"userID"];
...
- (void)getUserProfileDetail
{
    ...
    [request setPostValue:[[NSUserDefaults standardUserDefaults] objectForKey:@"userID"] forKey:@"profile_userid"];
...
//    NSLog(@"Response === %@", items);
    ...
    if (flag == 1) {
        ...
        if ([[[items valueForKey:@"status"] valueForKey:@"Status_code"] isEqualToString:@"001"]) {
            @try {
                NSDictionary *dict = [[[items valueForKey:@"status"] valueForKey:@"data"] objectAtIndex:0];
                NSDictionary *dictData = [[dict objectForKey:@"profile"] objectAtIndex:0];
                self.user = [[User alloc] initWithDictionaryForProfileDetail:dictData];
                
                NSString *followingCount = [NSString stringWithFormat:@"%@",[dictData objectForKey:@"follower_count"]];
                NSString *followerCount = [NSString stringWithFormat:@"%@",[dictData objectForKey:@"num_following"]];
                NSString *reviewCount = [NSString stringWithFormat:@"%@",[dictData objectForKey:@"review_count"]];
                NSString *avg_rating = [NSString stringWithFormat:@"%.2f",[[dictData valueForKey:@"avg_rating_profile"] floatValue]];
