...
- (void)itemsWillAppearFromButton:(UIButton *) button withFrame:(CGRect)frame inView:(UIView *)view {
	...
	while (currentItem <= itemCount) {
        ...
        UILabel *lblOffer = [[UILabel alloc] initWithFrame:CGRectMake(0, -26, frame.size.width, 20)];
        lblOffer.text = [self.delegate radialMenuName:self imageForIndex:currentItem];
        lblOffer.textAlignment = NSTextAlignmentCenter;
        lblOffer.textColor = [UIColor whiteColor];
        [lblOffer setFont:[UIFont fontWithName:HELVETICANEUEMEDIUM size:14.0]];
        [popupButton addSubview:lblOffer];
...
- (void) buttonDidFinishAnimation:(ALRadialButton *)radialButton {
    ...
    [appDelegate.tabBarController setSelectedIndex:appDelegate.previousTabIndex];
