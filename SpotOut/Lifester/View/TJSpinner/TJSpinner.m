...
#define k_SHADOW_OFFSET 0.05
#define k_CIRCE_EDGE_WIDTH 0.05
...
- (void)setOuterRadius:(CGFloat)outerRadius
{
    ...
    CGFloat newHeight = self.outerRadius*1.00+self.strokeWidth*1.00;
...
- (void) setRadius:(CGFloat)radius
{
    ...
    CGRect newFrameRect = self.frame;
    CGFloat newHeight = self.radius*1.00+self.thickness*1.00;
    CGFloat newWidth = newHeight;
...
+ (TJSpinner *)showHUDAddedToWindow:(UIWindow *)view
{
    TJSpinner *spinner = [[TJSpinner alloc] initWithSpinnerType:kTJSpinnerTypeActivityIndicator];
    spinner.frame = view.bounds;
    spinner.hidesWhenStopped = NO;
    [spinner setColor:[UIColor colorWithRed:247.0/255.0 green:148.0/255.0 blue:30.0/255.0 alpha:0.32]];
    
    [spinner setInnerRadius:12];
    [spinner setOuterRadius:15];
    [spinner setStrokeWidth:0.7];
    [spinner setNumberOfStrokes:12];
    [spinner setPatternLineCap:kCGLineCapRound]; //
    
    [spinner setRadius:5.0];
    ...
    CGRect allRect = view.bounds;
	// Draw rounded HUD backgroud rect
	CGRect boxRect = CGRectMake(roundf((allRect.size.width - 60) / 2) + 0,
								roundf((allRect.size.height - 60) / 2) + 0, 60, 60);
    UIView *HUD = [[UIView alloc] initWithFrame:boxRect];
    ...
    return MB_AUTORELEASE(spinner);
}

+ (TJSpinner *)showHUDAddedTo:(UIView *)view
{
    TJSpinner *spinner = [[TJSpinner alloc] initWithSpinnerType:kTJSpinnerTypeActivityIndicator];
    spinner.frame = view.bounds;
    spinner.hidesWhenStopped = NO;
...
+ (BOOL)hideHUDForView:(UIView *)view {
    UIView *overLayView = [view viewWithTag:1001];
    [overLayView removeFromSuperview];

    UIView *HUD = [view viewWithTag:1002];
    [HUD removeFromSuperview];

	TJSpinner *hud = [TJSpinner HUDForView:view];
	if (hud != nil) {
		[hud removeFromSuperview];
		[hud stopAnimating];
		return YES;
...
+ (TJSpinner *)HUDForView:(UIView *)view {
	Class hudClass = [TJSpinner class];
	NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
	for (UIView *subview in subviewsEnum) {
		if ([subview isKindOfClass:hudClass]) {
			return (TJSpinner *)subview;
		}
	}
	return nil;
}

+ (BOOL)hideHUDForWindow:(UIWindow *)view
{
    UIView *overLayView = [view viewWithTag:1001];
    [overLayView removeFromSuperview];
    ...
    TJSpinner *hud = [TJSpinner HUDForWindow:view];
	if (hud != nil) {
		[hud removeFromSuperview];
		[hud stopAnimating];
		return YES;
...
+ (TJSpinner *)HUDForWindow:(UIWindow *)view {
	Class hudClass = [TJSpinner class];
	NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
	for (UIView *subview in subviewsEnum) {
		if ([subview isKindOfClass:hudClass]) {
